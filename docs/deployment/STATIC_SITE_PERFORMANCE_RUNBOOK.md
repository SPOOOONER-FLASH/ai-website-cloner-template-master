...
2.3 的方案 A 已由甲方在宝塔面板实施：脚本加入 commit 比较护栏，执行周期调整。以下为实测验收。

### 7.1 循环已停止 —— 通过

| 检查项 | 修复前 | 修复后 |
|---|---|---|
| `out/index.html` mtime / inode | 每 5 秒改变，inode 每次不同 | 45 秒内 10 次采样**完全不变** |
| 外部 ETag（6 次 HEAD，间隔 8 秒） | 每 5 秒改变 | 6 次**全部相同** |
| 携带旧 ETag 的条件请求 | 约 12 秒后由 304 变 200 | 20 秒后仍返回 **304** |
| 服务器负载 / IO 等待 | 未取基线 | load 0.03，%wa 4.2 |

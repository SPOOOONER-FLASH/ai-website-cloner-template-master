...
- **轮询间隔定为 5 分钟。** 甲方 2026-08-26 决定：1 分钟「刷太快了」，5 分钟够用。
  这是一个体验取舍，不是技术限制——加护栏后 1 分钟同样安全，但没有必要。
  **两位 Agent 不要再自行改动这个值**，它是甲方明确定过的。
- 相应地，「push 到线上可见」的最大延迟是 5 分钟。向甲方汇报已上线时要按这个口径说，
  不要在 push 完立刻宣称线上已生效。
...
- ~~仓库根目录 20 项陈旧站点副本（见 2.4）尚未清理。~~ **已完成** —— 2026-09-07
  复查，根目录只剩 `out/`（12,191 个文件）与 `out-rayen/`（3,684 个文件），
  两者都是当前在用的构建产物，不是副本。

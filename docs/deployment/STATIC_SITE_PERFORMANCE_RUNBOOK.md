...
## 7. 修复结果与修复后基线（2026-08-25 已实测）

目前只调整了执行周期，**护栏尚未加入**。以下为实测验收。

### 7.1 循环未修复，仅被节流 —— 未通过

> ⚠️ **更正（2026-08-26）**：本节初版曾写“循环已停止 —— 通过”，那是**错误结论**。
> 当时的采样窗口只有 45 秒，短于实际执行周期，恰好落在两次执行之间，因此观察不到变化。
> 采样窗口必须长于调度周期，否则无法区分“已停止”与“周期变长”。

实际状态：宝塔任务的执行周期由 5 秒改为 **10 分钟**，但脚本内容仍是无护栏版本 ——
`git reset --hard origin/main` 与 `chown -R www:www .` 依旧无条件执行。

决定性证据（2026-08-26 17:44 采样）：

```text
任务日志       ★[2026-08-26 17:40:03] Successful      ← 每 10 分钟整点对齐
out/index.html          mtime = 17:40:02
out/contact/index.html  mtime = 17:40:02
out/company/index.html  mtime = 17:40:02
AGENTS.md               mtime = 17:40:02
```

全部文件的 mtime 与任务执行时刻同秒，说明**每 10 分钟仍然重写全部 3,833 个文件**。

| 指标 | 原始 | 当前 | 加护栏后应达到 |
|---|---|---|---|
| 全量重写频率 | 每 5 秒 | 每 10 分钟 | 仅在真有新提交时 |
| 浏览器缓存有效期 | 约 5 秒 | 约 10 分钟 | 直到下次实际发布 |
| push 到上线延迟 | ≤5 秒 | **≤10 分钟** | ≤30 秒（护栏使短周期几乎免费） |

改善是真实的（重写频率降低约 120 倍），但**根因未消除**，且付出了上线延迟从 5 秒变成
10 分钟的代价。当前配置在两个目标上都不是最优：缓存仍被周期性清空，发布也变慢了。

**待办：把 2.3 的护栏脚本填入宝塔任务的「脚本内容」框**，随后即可把周期调回 30 秒或 1 分钟，
同时获得稳定缓存与快速上线。验收方法见下方。

验收必须使用**长于调度周期**的采样窗口。以 10 分钟周期为例，至少连续观察 15 分钟；
或直接比对 `out/index.html` 的 mtime 与任务日志中最近一次 `★` 时间戳 —— 两者同秒即表示
该次执行仍在无条件重写。

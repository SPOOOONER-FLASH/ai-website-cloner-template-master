...
| # | 要什么 | 为什么卡 | 提出日期 |
|---|---|---|---|
| 1 | **6068 的产品照片文件** | ⚠ **图我看到了，文件不在硬盘上** —— 对话里是图片，下载文件夹里只有锈锁那批。**存成两个文件丢进一个文件夹告诉我路径，一条命令就上线**：`npm run assets:productphotos -- 6068-mortise-lever-handle-lock <文件夹>`，然后 `npm run assets:watermark`。<br>脚本会补白成 1000×1000 方板（**不裁** —— 锁体是横的，裁方会把锁舌切掉，而锁舌正是买家要看的），写进未打标的 `products/` 根目录，再由打标流程加 HYDE 标**并抹掉图上那个旧的红色 Hyland 椭圆标**（第一张图上就有）。顺序不能反。<br>记录里 `heroImage.src` 一填上它立刻发布 —— `withheld-products.test.ts` 写着「可见性只由 heroImage.src 决定」。119 曝光 / 3 询盘 / 23% 转化，全表回报最高 | 2026-09-11 |

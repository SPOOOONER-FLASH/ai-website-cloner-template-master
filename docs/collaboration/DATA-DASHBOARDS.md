...
5. **GA4 事件里还没有询盘**：09-22 之前 `generate_lead` 还不存在（表单用 fetch 提交，GA4 看不到）。
   事件已上线，甲方需要在 GA4 把它标成「关键事件」。阅读深度等新事件见 GTM 一节，上线后下期可见。
6. **旧站 URL 仍在 Google 里有展示**（`index.php?m=home&c=View&a=index&aid=1605` 71 次，
   `tid=131` 63 次）。301 是否把它们送到了对的新页，SEO 审计会逐条核对。

...
1. 宝塔面板 → 左边菜单 **计划任务**（有的版本叫「定时任务」）
2. 找到跟 cantonlock 有关的那条，点 **编辑** 或 **脚本内容**
3. 里面的文字全选复制，发我
...
**成功的样子**：你发我一段脚本，几行到几十行。里面大概率有 `git pull`，可能还有 `cp`、`rsync`、`npm run build`。

**我要看什么**：如果是 `cp -r` 这种整目录复制，每复制一次所有文件时间戳都刷新，即使内容没变 —— 那就对上了，改成保留时间戳的写法，一行的事。

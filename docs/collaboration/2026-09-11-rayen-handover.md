...
**状态**：本次全部工作**已提交并推送**（最后一个提交 `5d49426ca9`）。线上和磁盘一致。
接手可以直接从第 7 节开始干活。
...
| 项 | 提交 | 状态 |
|---|---|---|
| 518 张在售产品图打标，固定右下角，深浅两种墨按底色自动选 | `5d49426ca9` | ✅ 已推送 |
| 交接文档（本文件） | `cec4dd7f69` | ✅ 已推送 |

**提交时撞上的事，值得记一笔**：另一个会话（在做西语排版）留下了一个 `.git/index.lock`，
零字节、十四分钟没动过、HEAD 也没前进 —— 是**僵尸锁**，不是有人正在写。
判断方法：锁文件 mtime 有没有变、HEAD 有没有前进、`tasklist | grep git` 是不是真的在跑。
确认之后把它**移到 `tmp/claude-stale-lock/` 而不是删掉**，再正常提交。

**别拿真提交去试探锁**（第 8 节第一条就是这么来的），只用 `ls .git/index.lock`。
...
首页那句话已经承诺了「每个型号都有独立规格表：材质、尺寸、中心距、安装孔径、重量和表面处理」。
**买家点进去发现说好的一栏不存在，损失的不是这一句，是整张表的可信度。**
要么把数据补上，要么把这句话改短 —— 两者选一，别让它空着。
（2026-09-10 已经因为「背距」踩过一次：目录里大部分是拉手，拉手没有背距。）
...
**没有 amend**，因为同一分支上还有别的会话在干活，改写已推送的历史会连累他们。
...
英文版、缩放修复、图纸日文、中文文案重写、518 张产品图打标 —— **全部已提交推送**
（到 `5d49426ca9` 为止），线上磁盘一致。
接手直接做两件事：artunion 按型号抓规格（**不抓价格**），继续磨中文文案。
七条缺料在甲方手上，别自己编。**还没 purge —— 提醒甲方。**

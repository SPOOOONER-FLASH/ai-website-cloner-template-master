...
| 8 | 文章改写第 2 批和第 3 批（见第二节） | 09-24 完成：六篇的西语、葡语都已逐段通读。锁体对比指南补齐三语，摘要和 FAQ 交给生成器。锁芯/执手背距那篇的背距表是手打的，已经过期，数字已改正，三行登进计数测试。统一了万能钥匙那篇的 CK/GMK/GGMK 西语叫法，饰板那篇西语 FAQ 与正文用词对齐。全站扫了一遍：葡语文章里的葡萄牙用词（25 个文件）、西语里的西班牙用词、失实说法（自有 EN 1125 文件、我们的地弹簧） |
| 8b | `cylindrical-and-tubular-lock-comparison-2026`、`exit-device-comparison-2026` 两篇指南只有英文（exit-device 09-24 已补西葡）（GUIDES_WITHOUT_ES/PT），和锁体对比那篇一样，是被 AI 引用的对比页。先查有没有生成器管它们的表格，再照锁体那篇的办法补西语和葡语 | 待做 |

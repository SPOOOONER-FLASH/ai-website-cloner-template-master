...
## 修复已实施并验证（2026-08-25 收尾）

甲方已在宝塔面板为该任务加上 commit 比较护栏。实测：`out/index.html` 的 mtime 与 inode
45 秒内不再变化；外部 6 次 HEAD 的 ETag 完全一致；20 秒后携带旧 ETag 的条件请求返回 304。
循环已停止，缓存前置条件具备。完整修复后基线见 runbook §7。

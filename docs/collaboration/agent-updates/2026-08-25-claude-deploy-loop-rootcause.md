...
## 修复状态：已完成并验证（2026-08-26）

护栏脚本已填入宝塔任务 `canton update`，实测生效。

**验证**：任务在 18:10:01 确实执行（日志有 `★` 记录），而 `out/index.html` 的 mtime
仍为 18:00:03 未变 —— 该次执行走了护栏分支，未重写任何文件。外部 ETag 多次采样保持不变。

### 过程中的一次错误结论，值得记住

本节初版曾断言“循环已停止”，那是**错的**（已在 74483bbd 更正）。当时采样窗口仅 45 秒，
短于实际 10 分钟的调度周期，恰好落在两次执行之间。

教训：验证周期性行为时，只看“文件有没有变”不够，**必须同时确认该时段内任务确实执行过**。
最稳的方法是比对任务日志最近一次 `★` 时间戳与文件 mtime —— 同秒即表示仍在无条件重写。
这个方法不依赖采样时长。已写入 runbook §7.1.1。

# 【历史存档】操作手册 2026-08 ~ 2026-09-17

> ⚠ **这份文件不是指令，是记录。不要照着做。**
>
> 当前要做的事只在 `docs/collaboration/CLIENT-RUNBOOK.md` 里，那份文件永远只写
> 「现在」。这里保留的是过程：哪一天出了什么事、当时怎么处理的、为什么那样处理。
> 下面很多章节**已经做完或已经过时**，其中至少三处（1a / 1d / 1b）写的是
> 「这一节的规则又旧了，再跑一次第 1 节」—— 那正是这份文件被拆开的原因。
>
> 2026-09-17 实测：`index.php?lang=es` 已经 301 到 `/es/`，所以第 1b 节那个
> 「只做一次」的手工改动**已经做完了**。
...
| 仓库里最新的 | **144** |
| 新增的是什么 | DS011 门吸改名 + 逃生器械 16 个型号改名；**2026-09-17 起每条规则在英语、西语、葡语各出一份** |

> **2026-09-17 新增，这一次一定要跑。**
>
> 谷歌 9-17 的 404 导出里有两条是我们自己的页面，而且是几天前才抓的：
>
> ```
...
> ```
>
> 这两个型号改过名，**英文那边一直跳得好好的**，西语这边 404 —— 因为之前每条
> 跳转规则都是 `/products/` 开头，没有一条带语言前缀。谷歌手里有这两个西语地址
> （它们就在导出里），所以一次在一棵树上处理干净的改名，在另一棵树上丢了两个
> 已收录的页面。葡语站 9-16 上线，本来会原样继承这个洞。
>
> 现在规则从 48 条变成 144 条（英/西/葡各 48）。**规则在仓库里改好了，不跑这两行
> 命令它们不会生效。**
...
## 3b. Clarity 里怎么一眼分清爬虫和真买家

2026-09-10 甲方问「一直有个新加坡的在看我们网站，一直在线一整天」。
结论是爬虫，判断方法记在这里，因为这个问题每个月都会再问一次。

### 看五个字段，不用看录屏

| 字段 | 爬虫 | 真买家 |
|---|---|---|
| **Duration** | 1–3 秒 | 十几秒到几分钟 |
| **Clicks** | **0** | 至少 1 次 |
| **Pages** | **1**，Entry 和 Exit 是同一页 | 2 页以上 |
| **User ID** | **每次都不一样** | **回访时不变** |
| 落地页 | 直接落在深层产品页，不经类目页 | 从首页或类目页进来 |

**User ID 是最硬的一条。** Clarity 靠 cookie 认人：真人回访 ID 不变，
**不保存 cookie 的爬虫每抓一次就换一个新 ID**。所以看到七个会话七个不同 ID、
每个都停 1 秒 0 点击，不用再看别的了。

直接落在 `lc7065s`、`9217-bnac` 这种深层网址而不经列表页，说明它在
**照着我们的 sitemap 逐条抓** —— 那是机器的行为，人不会背网址。

### 想 100% 坐实：服务器上一条命令

Clarity 看不到 user-agent，服务器日志能看到。在宝塔终端里：

```bash
awk -F'"' '{print $6}' /www/wwwlogs/cantonlock.com.log | sort | uniq -c | sort -rn | head -25
```

打印访问量最大的 25 个 user-agent。**爬虫基本都会自己报名字** ——
`GPTBot`、`ClaudeBot`、`PerplexityBot`、`Bytespider`、`SemrushBot`、
`AhrefsBot`、`python-requests`、`Go-http-client`、`Scrapy`。

> 如果日志路径不对，先跑 `ls /www/wwwlogs/ | head` 看实际文件名，发我截图。

### 爬虫要不要拦

**不要。** 一天几个请求不消耗什么，而且 GPTBot / ClaudeBot / PerplexityBot
是我们**主动希望它来的** —— AI 引用就靠它们抓。robots.txt 里这几个是明确 Allow 的。

**唯一要做的事是：别把它当线索去跟进。**

---

## 3d. 爬虫要不要封 —— 2026-09-10 的完整结论

甲方 2026-09-10 想封掉「一直在线的新加坡访客」。**结论是不要按国家封**，
理由和替代做法都在这里，因为这个决定每隔几个月就会重新提一次。

### 先分清两种日志，否则怎么查都查不对

| 你在哪看 | 它看到的是谁 |
|---|---|
| **Clarity / Google Analytics** | **真实访客**。它们是浏览器里跑的 JS，拿到的是访客自己的位置。**Clarity 说新加坡，人就真在新加坡** |
| **服务器 nginx 日志** | **Cloudflare 回源节点**。多伦多、洛杉矶、新加坡都是 CF 的节点位置，**和访客在哪毫无关系** |

**这是最容易搞错的一点。** 服务器日志里那些 `104.22.x.x`、`172.64–71.x.x`、
`162.158.x.x` 全是 Cloudflare 自己的 IP。在服务器上 `iptables` 封它们，
封掉的是 Cloudflare，不是爬虫。

### 那批扫 .php 的请求：无害，不用管

日志里会看到大量 `/zz.php`、`/wp-admin/`、`/.env` 之类的请求，**全部 404**。

**我们的网站是静态导出的，服务器上没有任何 PHP 在执行。** 那是僵尸网络在全网
无差别扫已经被挂马的 WordPress 站点，我们不是 WordPress，它扫不到东西。
每个公网站点每天都有这种流量。**不需要做任何事。**

### 为什么不按国家封新加坡

1. **新加坡是目标市场。** 需求数据里东南亚曝光 21 次，与欧洲并列第一，
   高于美国的 13 次。新加坡是东南亚门五金的转口贸易枢纽。
2. **在服务器上封不掉**（见上面那张表）。
3. **代价不对等**：换来每天少 7 条脏数据，代价是可能拦掉一个真买家，
   而且你永远不会知道丢了谁。

### 真正的代价是分析数据被污染

一天 7–11 次访问对服务器毫无负担。它的伤害是进了 GA 的新用户计数、
拉低平均停留时长、拉高跳出率。**解决这个不需要封任何人。**

### 建议的三步，按顺序

**① 开 Bot Fight Mode（免费，一个开关，最有效）**

Cloudflare → **Security** → **Bots** → **Bot Fight Mode** 打开。

它自动挑战非认证的自动化流量，**同时放行 Googlebot / Bingbot / ClaudeBot /
PerplexityBot 这些已验证爬虫** —— 那几个是我们主动希望它来的，AI 引用就靠它们抓，
robots.txt 里也是明确 Allow 的。
...
```
(ip.geoip.country eq "SG" and http.request.uri.path contains "/es/products/")
```

Action 选 **Managed Challenge**，**不要选 Block**。

- 真人 0.5 秒无感通过
- 脚本过不去
- **万一真有新加坡买家，他不会被挡在门外**

用 `/es/`（西语）路径是因为那个爬虫只抓西语页面，而新加坡买家极不可能看西语站 ——
这个条件本身就把误伤降到最低。

**③ 在 GA4 里把它过滤掉（这才真正解决「一直在线」的观感）**

GA4 → 管理 → 数据设置 → 数据过滤器，或者在报告里加比较条件排除：
**国家 = 新加坡 且 互动时长 < 3 秒 且 页面数 = 1**。

### 唯一值得在服务器上封的那种

**直连源站、绕过 Cloudflare 的 IP。** 2026-09-10 查到一个：

```
207.174.5.233  洛杉矶  AS153371 BACK WAVES LIMITED（机房）
```

它伪装成 Edge 浏览器，但住宅用户不会落在这种小众 IDC 的 AS 上。甲方当时决定不封。
**如果以后要封这类直连 IP：**
...
> ⚠ 第二行不能省，否则服务器一重启规则就没了。

### 一个以后值得做、但不急的改进

服务器上 nginx 没有配 `real_ip`，所以日志里记的全是 Cloudflare 节点 IP。
配了之后日志才能看到真实访客 IP，日志分析才有意义。
**这不是今天的紧急事** —— 现在要判断访客真假，看 Clarity 比看日志准。
要做的时候跟我说，我给完整配置和验证步骤。

---

## 3c. BAU 展商目录里的官网链接（免费的高质量外链）

2026-09-10 在 Clarity 里发现一个 referrer：`exhibitors.bau-muenchen.com`。

那是 **BAU 慕尼黑展的展商后台**（Exhibitor Shop，订展位和服务的地方），
不是公开目录。所以那次访问很可能是你自己在后台里点了一下公司资料上的网站链接。

**但它提醒了一件值得做的事。**

Bing 报告里唯一我们没修的一条是「**来自高质量域的入站链接不足**」——
那是网站本身改不了的，只能靠别的站链过来。而 BAU 是世界建筑材料第一展，
`bau-muenchen.com` 的域名权重很高。

### 你要确认的

下次登录 BAU 展商后台时，检查公司资料里的**官网字段**：

| 应该填 | 不该填 |
|---|---|
| `https://cantonlock.com` | 旧的 `index.php` 地址 |
| | `stahlock.com`（那是另一个店，权重不该给它） |
| | 空着 |

然后去公开目录页
`bau-muenchen.com/en/trade-fair/exhibitor-directory/exhibitors-brands/`
搜 Canton Hyland，确认列表页上那条链接指向 cantonlock.com。

**这是一条不花钱、不用求人的高质量外链。** 同一件事在广交会和巴西展的
展商系统里也要做一遍 —— 每个展会的官网都是高权重域。

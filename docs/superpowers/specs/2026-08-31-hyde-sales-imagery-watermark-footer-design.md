...
- 本设计只覆盖第一批收尾：6 个首页图片位置、1,485 张产品图 HYDE 品牌复核、Footer 精简与标题评审。
...
先为 6 个位置各生成 3 张候选，共 18 张。候选是预览材料，不直接进入正式站。用户选择后，只将
6 张定稿复制到 `public/images/editorial/`，生成响应式 WebP，并更新英文、西语、分享图、alt 和
`IMAGE_CREDITS.md`。
...
| Panic Exit Devices 首页首图 | `home-commercial-egress.webp`，并复核 `home-panic-exit-bars.webp` 的首页/分享图用途 | 门、门框、横向 panic bar 和锁头关系清楚可见；产品是主体，不是建筑背景中的小点 |
...
- 6 张定稿原尺寸检查、18 张候选用途对照、响应式候选完整性；

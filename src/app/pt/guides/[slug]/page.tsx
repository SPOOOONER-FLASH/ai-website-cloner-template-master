...
export async function generateMetadata({ params }: GuidePageProps): Promise<Metadata> {
  ...
  return {
    ...
    */
    alternates: {
      canonical: `/pt/guides/${slug}/`,
      languages: alternateLanguages(`/guides/${slug}`),
    },
    openGraph: {
      type: "article",
      url: absoluteUrl(`/pt/guides/${slug}/`),
      title,
      description,
...
export default async function GuideArticlePage({ params }: GuidePageProps) {
  ...
  return (
    <>
      <NewsArticleJsonLd article={article} locale="pt" section="guides" />
      <ArticleFaqJsonLd article={article} locale="pt" />
      <NewsDetail article={article} locale="pt" section="guides" />
    </>
  );
}

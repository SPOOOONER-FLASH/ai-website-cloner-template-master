  ...
  for (const category of getTopLevelCategories()) {
    for (const child of category.children ?? []) {
      ...
      out.push({
        ...
        category: {
          slug: category.slug,
          name: category.namePt ?? category.name,
          image: { src: category.image.src ?? "", label: category.image.label },
        },
        child: { slug: child.slug, name: child.namePt ?? child.name },
      });
    }
  }
  return out;
...
export async function generateMetadata({ params }: CollectionPageProps): Promise<Metadata> {
  ...
  /*
    46-character budget, same as everywhere else. Portuguese names run longer than the
    English ones — "Fechaduras cilíndricas de uso pesado" is 35 characters before any
...
export default async function ColeccionPage({ params }: CollectionPageProps) {
  ...
  const url = absoluteUrl(`/pt/collections/${collection.slug}/`);
    ...
    <>
      <JsonLd
        data={itemListSchema(
          ...
          items.map((p) => absoluteUrl(`/pt/products/${p.categoryPath[0]}/${p.slug}/`)),
        )}
      />
      <JsonLd
        data={breadcrumbSchema([
          ...
          { name: "Produtos", url: absoluteUrl("/pt/products/") },
          {
            ...
            url: absoluteUrl(`/pt/products/${collection.category.slug}/`),
          },
      ...
      <main className="isolate mt-48 flex-grow justify-self-start lg:mt-192">
        ...
        */}
        {(() => {
          const ranges = collectionSpecRanges(items, "pt");
                    ...
                    {ranges.map((range) => (
                      ...
                      >
                        ...
                        <dd className="text-c2 text-ink-tertiary">
                          {statedOn(range.stated, items.length, "pt")}
                        </dd>
                      </div>
                    ))}
                  </dl>

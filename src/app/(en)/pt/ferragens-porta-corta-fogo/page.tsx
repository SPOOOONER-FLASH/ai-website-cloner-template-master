...
/*
  Metadata is written out rather than taken from `pageMetadata`.

  That helper builds an en/es pair and decides hreflang from `hasSpanishMirror`. This page
  has no Spanish mirror and no English twin — it is a single Portuguese document — so
  running it through the pair-builder would either claim alternates that do not exist or
  need the helper widened to a third locale for one page. Widening it is the change to make
  when there IS a Portuguese section; today it would be a locale abstraction with one
  member.
*/
...
export default function FerragensPortaCortaFogoPage() {
  return (
    <main className="isolate mt-48 flex-grow justify-self-start lg:mt-192" lang="pt-BR">

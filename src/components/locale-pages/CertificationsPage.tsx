...
export function certificationsMetadata(locale: Locale): Metadata {
...
export function CertificationsPage({ locale }: { locale: Locale }) {
  ...
  const fields = [
    [tx(locale, "Exact model scope"), "coversModel"],
    [tx(locale, "Issuer"), "issuer"],
    [tx(locale, "Document reference"), "reference"],
    [tx(locale, "Issue date"), "issued"],
  ] as const;
      ...
      <div className="layout space-y-96 lg:space-y-136">
        <section className="col-content grid grid-cols gap-x gap-y-48">
          <div className="col-span-full">
            <Breadcrumbs
              items={[{ label: tx(locale, "Home"), href: p("/") }, { label: tx(locale, "Certifications") }]}
            />
          </div>
          <div className="col-span-full lg:col-span-5 xl:col-span-9">
            <p className="text-kicker uppercase tracking-[0.14em] text-ink-secondary">
              {tx(locale, "Quality evidence")}
            </p>
            <h1 className="mt-16 text-h1 text-ink">
              {tx(locale, "Certificates and test reports. What is tested, and in whose name.")}
            </h1>
          </div>
        ...
        <section className="col-content border-t border-line" aria-labelledby="certificate-register">
          <h2 id="certificate-register" className="sr-only">
            {tx(locale, "Certificate register")}
          </h2>
          {certificates.map((certificate, index) => (
            <article
              key={`${certificate.reference}-${certificate.coversModel}`}
            ...
            >
              <div className="col-span-2 sm:col-span-1 md:col-span-2 xl:col-span-3">
                ...
                <p className="mt-16 text-c2 font-semibold uppercase tracking-[0.08em] text-ink">
                  {tx(locale, "Verified record")}
                </p>
              </div>
              <div className="col-span-full sm:col-span-3 md:col-span-5 xl:col-span-10">
                <h2 className="text-h2 text-ink">{tx(locale, certificate.title)}</h2>
                <p className="mt-16 max-w-[60ch] text-c1 text-ink-secondary">
                  {tx(locale, "The details are public. The full report is sent on request, complete, as the issuer requires.")}
                </p>
              </div>
        ...
        <section className="col-content grid grid-cols gap-x gap-y-32 border-t border-line pt-32">
          <h2 className="col-span-full text-h2 text-ink lg:col-span-5 xl:col-span-9">
            {tx(locale, "Check the report before you specify the part.")}
          </h2>
          <div className="col-span-full lg:col-span-5 lg:col-start-7 xl:col-span-9 xl:col-start-16">
            ...
            <div className="mt-24">
              <ArrowLink href={p("/contact")}>{tx(locale, "Request technical documents")}</ArrowLink>
            </div>
          </div>
        </section>
      </div>

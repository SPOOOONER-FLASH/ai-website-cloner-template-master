...
export function ArticleBody({ blocks, locale }: { blocks: ArticleBlock[]; locale: Locale }) {
  return <div className={styles.body} id="article-overview">{blocks.map((block, index) => {
    ...
    if (block.kind === 'table') {
      ...
      return <DataTable key={index} locale={locale} caption={block.caption || { en: 'Reference table', es: 'Tabla de consulta', pt: 'Tabela de consulta' }[locale]} columns={block.headers.map((label, column) => ({ label, sort: numeric[column] ? 'number' : 'text' }))} rows={block.rows.map(row => row.map((text, column) => ({ text, value: numeric[column] ? Number(text) : undefined })))} />;
    }

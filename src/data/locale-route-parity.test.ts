...
/**
 * The market trees, 2026-09-24.
 *
 * Seven locales × seven pages, generated by scripts/scaffold-market-routes.mjs from
 * MARKET_MIRROR_PATHS. The same two assertions as the Portuguese tree above, for the same
 * reason: hreflang (src/lib/market-mirror.ts) advertises every one of these routes on the
 * English page it mirrors, so a market route missing from disk is forty-nine 404s in the
 * sitemap — and a route on disk that the list does not know about is invisible to the
 * language menu.
 */
test("every market locale has exactly the seven market routes on disk", async () => {
  const { marketLocales, MARKET_MIRROR_PATHS } = await import("./market-locales.ts");
  const expected = [...MARKET_MIRROR_PATHS].filter((route) => route !== "/").sort();
  for (const code of marketLocales) {
    const onDisk = routesUnder(code);
    ...
    assert.deepEqual(onDisk, expected, `/${code}/ routes differ from MARKET_MIRROR_PATHS`);
  }
});

test("every market route mirrors an English route that exists", async () => {
  const { MARKET_MIRROR_PATHS } = await import("./market-locales.ts");
  const englishRoutes = new Set(topLevel([...routesUnder("(en)"), ...routesUnder("")]));
  for (const route of MARKET_MIRROR_PATHS) {
    if (route === "/") continue;
    assert.ok(englishRoutes.has(route), `${route} is declared a market mirror but has no English page`);
  }
});

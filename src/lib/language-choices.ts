...
/** Kept for callers that imported it while the seven were a separate tier (2026-09-24). */
...
export function languageChoices(pathname: string, locale: Locale): LanguageChoice[] {
  ...
  const englishPath =
    (locales as readonly string[]).includes(prefix) && prefix !== "en"
  ...
  return locales.map((code) => {
    /*
      ...
      a Portuguese-only page while the panel offered something else. Ten locales since
      2026-09-25; the seven overlay locales carry the Portuguese route set.
    */

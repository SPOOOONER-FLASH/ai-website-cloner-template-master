...
/**
 * The whole site's sitemap entries — /sitemap.xml (src/app/sitemap.ts) emits all of them,
 * and /<locale>/sitemap.xml (src/lib/locale-sitemap-xml.ts) emits one locale's slice for that
 * locale's Search Console property. One data source, two shapes.
 *
...
export function siteSitemap(): MetadataRoute.Sitemap {
  if (!indexable) return [];

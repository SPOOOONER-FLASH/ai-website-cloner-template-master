...
/**
 * A guide written in English whose translation has not been done yet (client, 2026-09-22:
 * translations deferred) has no /es or /pt page. Both mirror functions ask this before the
 * prefix rules, so the sitemap, hreflang, the language switch and every localisedHref()
 * agree without each needing to know about guides. The lists are generated by
 * scripts/build-content-index.mjs from the content files themselves.
 */
function untranslatedGuide(clean: string, lacking: readonly string[]): boolean {
  const match = /^\/guides\/([^/]+)$/.exec(clean);
  return match !== null && lacking.includes(match[1]);
}
...
export function hasSpanishMirror(enPath: string): boolean {
  ...
  if (untranslatedGuide(clean, GUIDES_WITHOUT_ES)) return false;
...
export function hasPortugueseMirror(enPath: string): boolean {
  ...
  if (untranslatedGuide(clean, GUIDES_WITHOUT_PT)) return false;
  return PORTUGUESE_MIRROR_PREFIXES.some(
    (prefix) => prefix !== "/" && (clean === prefix || clean.startsWith(`${prefix}/`)),
  );
}

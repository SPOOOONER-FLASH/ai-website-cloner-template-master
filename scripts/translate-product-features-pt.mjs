#!/usr/bin/env node
/**
 * Writes `featuresPt` on the product records whose feature list is COMPLETELY translated.
 *
 * ---------------------------------------------------------------------------
 * ALL OR NOTHING, PER RECORD
 *
 * A record with four Portuguese bullets and two English ones is a list that looks finished
 * and is not — the same failure the article bodies avoid by falling back whole. So a record
 * is written only when every line of it resolves in src/data/pt-features.ts, and the rest
 ...
 *
 * Usage:
 *   node scripts/translate-product-features-pt.mjs           # report only
 *   node scripts/translate-product-features-pt.mjs --write
 *   node scripts/translate-product-features-pt.mjs --all     # every blocking line
 */
...
/*
  ...
  (see scripts/translate-products-pt.mjs and its `grab`).
*/
function loadTable() {
  const source = readFileSync("src/data/pt-features.ts", "utf8");
  const table = new Map();
...
for (const { file, data } of records) {
  ...
  if (!missing.length) {
    complete += 1;
    if (write) {
      data.featuresPt = data.features.map((line) => table.get(line));
...
console.log(
  `${write ? "wrote" : "would write"} featuresPt on ${complete} of ${records.length} records ` +
...
if (blocking.size) {
  ...
  console.log("\nAdd them to src/data/pt-features.ts.");
}
